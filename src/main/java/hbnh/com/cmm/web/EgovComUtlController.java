...
public class EgovComUtlController {
	 ...
	 * JSP 호출작업만 처리하는 공통 함수
	 */
	@RequestMapping(value = "/EgovPageLinkAction.do")
	public String moveToPageAction(@RequestParam("link") String linkPage, 
			HttpSession session, 
			@RequestParam(value = "baseMenuNoMain", required = false) String baseMenuNo,
			@RequestParam(value = "baseMenuNmMain", required = false) String baseMenuNm) {
		String link = linkPage;
		// service 사용하여 리턴할 결과값 처리하는 부분은 생략하고 단순 페이지 링크만 처리함
		if (linkPage == null || linkPage.equals("")) {
			link = "cmm/egovError";
